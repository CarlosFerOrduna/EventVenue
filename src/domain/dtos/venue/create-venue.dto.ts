...
export class CreateVenueDto extends Dto {
  private constructor(public name: string, public description?: string, public capacity?: number) {
    super()
  }
  ...
  static create(props: { [key: string]: any }): [string?, CreateVenueDto?] {
    ...
    if (!!capacity && capacity.trim().length < 1) return ['capacity is not valid']
