...
export class CreateUserDto extends Dto {
  private constructor(public email: string, public names?: string, public surnames?: string, public role?: Role) {
    super()
  }
  ...
  static create(props: { [key: string]: any }): [string?, CreateUserDto?] {
    ...
    if (!!surnames && surnames.trim().length < 1) return ['Surnames is not valid']
    if (!!role && role.trim().length < 1) return ['Role is not valid']
    if (!!role && !Object.values(Role).includes(role)) return ['Role is not valid']
